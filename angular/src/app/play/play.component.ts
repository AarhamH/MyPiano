...
export class PlayComponent extends AppComponentBase {
    ...
    @ViewChild('inpField', { static: true }) inpFieldElement: ElementRef;
    @ViewChild('tryAgainBtn', { static: true }) tryAgainBtnElement: ElementRef;
    @ViewChild('timeTag', { static: true }) timeTagElement: ElementRef;
    @ViewChild('mistakeTag', { static: true }) mistakeTagElement: ElementRef;
    @ViewChild('wpmTag', { static: true }) wpmTagElement: ElementRef;
    @ViewChild('cpmTag', { static: true }) cpmTagElement: ElementRef;
    ...
    loadParagraph() {
        const ranIndex = Math.floor(Math.random() * this.paragraph.length);
        this.typingText = this.paragraph[ranIndex];
        this.charIndex = this.mistakes = 0;
        this.isTyping = false;
      }

    initTyping() {
        let typedChar = this.inpField.split("")[this.charIndex];
        if (this.charIndex < this.typingText.length - 1 && this.timeTag > 0) {
          if (!this.isTyping) {
            this.startTimer();
            this.isTyping = true;
          }
          if (typedChar == null) {
            if (this.charIndex > 0) {
              this.charIndex--;
              if (this.typingText[this.charIndex] === '_') {
                this.mistakes--;
              }
            }
          } else {
            if (this.typingText[this.charIndex] === typedChar) {
              this.charIndex++;
            } else {
              this.mistakes++;
            }
          ...
          let wpm = Math.round(((this.charIndex - this.mistakes) / 5) / (60 - this.timeTag) * 60);
